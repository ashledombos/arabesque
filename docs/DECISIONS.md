...
| ~~`_reconcile_missed_exits` invente exit quand broker injoignable (2026-05-14)~~ ✅ **Corrigé 2026-05-17** | Au 1er restart 2026-05-14 07:39 UTC pendant la panne PriceFeed, FTMO inaccessible (5/5 `ALREADY_LOGGED_IN`+`CANT_ROUTE`). `_reconstruct_exit_from_history` a quand même logué `exit reconciled_stop_loss` ETHUSD SHORT cabriole avec `src=estimated_fallback bars=880` `mfe_r=0.0` `be_set=False`. Position en réalité toujours ouverte côté broker (vérifié au 2e restart 07:42 UTC). Cause racine : `_reconcile_missed_exits` mappait broker injoignable à `set()` vide, ne distinguant pas "broker répond avec 0 position" de "broker injoignable". | Faux exit dans `logs/trade_journal.jsonl` → pollue audit edge, déclenche `8 pertes consécutives` alert à tort, fausse les stats stratégie. Patché manuellement (event renommé `exit_invalidated_by_bug` + correction_note). | Fix 2026-05-17 : `broker_open_ids` typé `dict[str, set[str] \| None]`. `None` = injoignable → reconcile **différé** au prochain boot, aucun exit écrit. Couverture : `tests/test_reconcile_broker_unreachable.py` (3 tests : broker indispo diffère, broker répond vide reconcilie, mix FTMO indispo / GFT OK). |
| ~~Boucle `ALREADY_LOGGED_IN` cTrader, session fantôme côté serveur (2026-05-12 / 14 / 18)~~ ✅ **Corrigé 2026-05-18 (patch A+B)** | `ctrader.connect()` retry interne `[30, 60, 120, 120, 120]s` = 7.5 min sur `ALREADY_LOGGED_IN`. À chaque retry, appelait `_stop_client()` qui ne fait que `reactor.callFromThread(stopService)` — ferme le socket TCP **mais ne désauthentifie pas l'app côté cTrader**. La session reste "logged in" jusqu'à expiration serveur (TTL plusieurs minutes, observation 14-05 → 8h, 18-05 → 2h). Les 7.5 min de retry étaient sous ce TTL → tous les retries échouaient. `price_feed._run_loop` rebouclait avec backoff 5s→120s, rappelait `connect()`, refaisait 7.5 min de retry. Boucle infinie observable dans `logs/journal` 12-05/14-05/18-05. Pourquoi `systemctl stop && sleep 60 && start` opérateur résout : `disconnect()` envoie `UnsubscribeSpotsReq` + `stopService` propre, 60s suffit après ce cleanup réel. | Engine `active=yes` mais feed mort, watchdog v1 alerte mais n'auto-restart pas (decision Phase 4). Coût direct : -1R Glissade XAUUSD 14-05 (incident replay). Coût 18-05 : 0 signal manqué sur 03:00-05:30 UTC (marché asiatique calme). | Patch A+B `arabesque/broker/ctrader.py` (commit à venir) : **(A)** délais allongés `_ALREADY_LOGGED_IN_RETRY_DELAYS = (60, 180, 600)` (14 min total, sous Phase 4 freeze maintenance). **(B)** nouvelle méthode async `_cleanup_for_retry()` (unsubscribe spots + stopService + reset `_client=None`, `_connected=False`, `_subscribed_symbol_ids=set()`) appelée avant chaque retry à la place de `_stop_client()`. Autres exceptions (timeout, INVALID_REQUEST, etc.) gardent le chemin `_stop_client()` minimal. Couverture : `tests/test_ctrader_already_logged_in_retry.py` (6 tests : retry_delays figés, cleanup appelé sur ALREADY_LOGGED_IN, idempotence, reset état effectif, autre erreur inchangée, timeout inchangé). **Auto-restart watchdog explicitement différé** (réf. discussion 18-05) : papier collant qui masque ce root cause + risque de couper un trade en cours ; le patch A+B doit suffire, revue après 1 récidive éventuelle hors Phase 4. |
| ~~`be_set` à sémantique mixte (live vs reconcile) (2026-05-14)~~ ✅ **Corrigé 2026-05-17** | Le champ `be_set` du record exit mélangeait deux sémantiques distinctes : (1) **path live** (`position_monitor._check_breakeven`) → `be_set=True` UNIQUEMENT après `broker.amend_position_sltp` succès = état physique réel ; (2) **path reconcile** (`live._reconstruct_exit_from_history`) → `be_set = mfe_r >= 0.3` = inférence post-hoc depuis bars parquet, aucune preuve broker. Cas révélateur : trade XAUUSD glissade 14-05 (`ae845c5d-fb2`, FTMO Challenge) — engine FTMO down 09:47→17:23 UTC (panne PriceFeed), position entrée 09:01:24, fermée 17:24:45 par SL plein côté broker (`exit=4666.89` = `SL−0.05`). Journal affichait `be_set=true mfe_r=0.91 result_r=−1.002` faisant croire BE armé alors que `amend_position_sltp` n'avait JAMAIS été appelé (Phase 2.5 BE polling pas active à cette date). | Audits et invariants pollués : `be_unarmed_ratio` sous-estimait les BE non armés ; le bug "BE inféré mais loser" (signe pathognomonique d'un downtime engine) invisible. | Fix 2026-05-17 : nouveau champ `be_source` ∈ {`broker_armed`, `broker_evidence`, `inferred_from_mfe`, `not_armed`, `unknown`} reflète l'état RÉEL — **taxonomie stricte** : `broker_armed` RÉSERVÉ au path live (`position_monitor._check_breakeven` après succès observé `amend_position_sltp`) ; `broker_evidence` posé par le path reconcile quand `broker_detail` confirme `exit ≈ be_target` (preuve forte INDIRECTE — on déduit l'amend, on ne l'observe pas) ; `inferred_from_mfe` MFE parquet seul sans preuve broker (cf. pattern XAUUSD 14-05) ; `not_armed` ni preuve ni inférence ; `unknown` rétrocompat ancien record. `be_set` conservé pour rétrocompat. Nouveau champ ajouté à `live_monitor.record_exit` + JSONL. **Invariants** : `_be_was_armed_broker()` retourne True pour `broker_armed` ET `broker_evidence` (fallback `be_set` pour vieux records) — la distinction sert à la traçabilité d'audit, pas à invalider la preuve indirecte. Nouveau `be_inferred_but_loser` (ALERT≥1, CRITIQUE≥3) détecte le pattern XAUUSD 14-05. Couverture : `tests/test_be_source_semantics.py` (6 tests incluant régression XAUUSD 14-05 + cas `broker_evidence`). |
