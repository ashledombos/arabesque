...
|---|---|---|---|
| **TD-012** | `DryRunAdapter.get_account_info()` retournait 100 000 $ fixes — en P3 (cTrader dry-run), `_refresh_account_state()` aurait écrasé l'AccountState avec ces valeurs, rendant les guards DD aveugles | `DryRunAdapter.__init__` accepte `start_balance`, `_equity`/`_balance` trackés, `on_trade_closed(pnl)` ajouté à `BrokerAdapter` (no-op) et surchargé dans `DryRunAdapter` ; orchestrator appelle `broker.on_trade_closed(pnl)` dans `_update_account_on_close` | **2026-02-21** |
| **TD-013** | Spike de données UNIUSD : une bougie corrompue avec High ~56 (prix normal ~6.5) a généré un R=663.5 fantôme en replay (trade #1 par P&L, 84% des profits totaux) | Filtre anti-spike ajouté dans `_clean_ohlc` (`data.py`) : retire toute bougie dont High > median_close_20 × 3.0 ou Low < median_close_20 / 3.0, avec log WARNING pour audit | **2026-02-21** |
