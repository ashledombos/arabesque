...
h. **Espace disque** (la machine sert aussi de builder de paquets, 2026-07-04) : `df --output=avail -BG /var/home | tail -1` ≥ **30 Go** libres. Sous 30 Go → alerte (un disque plein fait échouer l'append `trade_journal.jsonl` = perte de données silencieuse). Sous 10 Go → 🚨 critique, recommander de purger les artefacts de build immédiatement.
...
| `portefeuille_ramp` | **Critère go phase portefeuille (remplace phase4_revalidation, 2026-07-03)** : compte uniquement les exits `strategy == glissade` (XAUUSD) depuis `2026-07-03T12:00:00+00:00`. Revue possible a `n ≥ 30`, decision cible a `n ≥ 50`. Lance (a) `python scripts/check_execution_invariants.py --since 2026-07-03 --per-broker` ; (b) `python scripts/replay_live_vs_theory.py --since 2026-07-03 --strategy glissade` ; (c) audit sizing avant toute hausse. | **Verdict combiné** : invariants `ok`, aucun incident integrite ouvert, sizing representatif et `meanΔR ≥ -0.10R` seulement permettent de proposer une hausse (validation user). Sinon rester au risque courant ou reduire. Si `n < 30`, no-op de decision et simple collecte (au débit actuel ~1-2 trades/mois, n≥30 = plusieurs mois — normal). Source d'autorite : `docs/VALIDATION_CONTRACT.md` / `config/validation_policy.yaml` (phase `portefeuille`, objectif ≥1,5R net/mois). |
| `candidate_pipeline` | **Suivi du candidat en instruction** (2026-07-03 : Renversé-métaux). Si un dry-run parquet ou un shadow est en cours (cf. HANDOFF section Immédiat), vérifier son avancement : dry-run → lire le log/jsonl du run ; shadow → compter les `👻 renverse shadow` dans les logs et agréger WR/Exp. | Inclure l'avancement dans le résumé (« candidat Renversé : shadow 12/100 trades, Exp +0.19R »). À ≥100 trades shadow OU fin du dry-run → proposer l'étape suivante du pipeline (validation user). Pas d'auto-promotion. |
| `ftmo_minlot_overshoot` | **Le déblocage FTMO est-il réel ?** (fenêtre DD -7→-8, sizing ~11$/trade). Compte sur 7j : ordres FTMO tentés vs rejets `risk overshoot` (`journalctl` + `logs/broker_guard_rejects.jsonl`). Si ratio rejets ≥ ~80% sur n ≥ 5 tentatives | Le déblocage est théorique — noter dans HANDOFF et proposer à l'opérateur : accepter (FTMO = venue morte de facto) ou ajuster. Pas d'auto-action. |
| `hl_funding_regime` | **Réveil du carry Hyperliquid en PARK** (2026-07-09, cf. `docs/audit/cash_and_carry_hl_protocole_2026-07-09.md`). Lance `python scripts/hl_funding_regime.py` (read-only, ~10 s réseau) : funding annualisé 14 j sur 7 instruments HL. Verdicts : `dormant` (normal — le funding de croisière ~5-12 %/an ne paie pas la marge de survie) / `wake` (≥3 instruments > 20 %/an) / `wake_confirmed` (wake soutenu ≥ 7 j → régime euphorique réel). Historique : `logs/hl_funding_regime.jsonl`. | Si `wake_confirmed` → noter dans HANDOFF + proposer à l'opérateur la réouverture du dossier cash-and-carry via **nouveau protocole conditionnel pré-enregistré** (jamais d'entrée directe). `wake` simple → mention informative dans le résumé. `dormant` → rien. Aucune action auto, aucun capital engagé. |
| `adage_ombre` | **Ombre « données » Adage — jalon 4 (2026-07-11, DECISIONS)**. Lance `python scripts/adage_ombre_daily.py` (fetch incrémental Dukascopy XAUUSD + rejeu chaîne Orchestrator complète, ~1-2 min ; `--no-fetch` si réseau indisponible). Lis le verdict dans `logs/adage_ombre_state.json` (seuils PRÉ-ENREGISTRÉS dans le script) : `collecte` (n_ombre < 30) / `revue_due` (n_ombre ≥ 30, ~6 sem) / `tripwire_dd` (maxDD net série complète < -16,2R = pire creux gravé à la dérogation DD dépassé). | `collecte` → 1 ligne informative (« Adage ombre : n=X, Exp net ombre ±Y »), pas de notif dédiée. `revue_due` → proposer la revue opérateur (go/no-go jalon 5 micro-live 0,20-0,30 %/session) + note HANDOFF. `tripwire_dd` → 🚨 alerte Telegram : l'hypothèse « creux dans la distribution » est factuellement invalidée, proposer KILL (pas d'auto-action). Sessions : `logs/adage_ombre_sessions.jsonl`. |
| `stale_bilan` | Pas de modif `logs/journal/YYYY-MM.md` depuis ≥ 8j | Note suggestion : « envisage `/bilan` ». Pas d'action auto. |
| `table_rase_due` | **Audit mensuel « à zéro » (skill `/table-rase`, actée opérateur 2026-07-18)**. Jour du mois ∈ [14..17] ET aucun fichier `docs/audit/table_rase_YYYY-MM.md` pour le mois courant. | **Proposer, ne PAS auto-lancer** (session lourde ~1-2h : sous-agent de dérivation à froid + falsification) : ligne dans l'output + notif si due — « 📅 Table rase mensuelle due — lance `/table-rase` ». Rien d'autre. |
