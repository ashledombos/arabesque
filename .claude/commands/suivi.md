...
| `missing_trades_unjustified` | Lance `python scripts/replay_signals_vs_live.py --since J-7`. Si `manquants > 2` pour une stratégie (hors weekend blocked et `strategy_broker_exclusions`) | Alerte Telegram+ntfy avec liste (stratégie, instrument, ts). Investigue : engine aveugle, drop signal silencieux, filtre trop restrictif. Pas d'auto-fix — diag manuel. |
