...
g. **Feed crypto FTMO vivant** : `journalctl --user -u arabesque-live.service --since "35 minutes ago" | grep -E "BarAggregator.*(BTCUSD|ETHUSD)" | tail -1`. Les cryptos tournent 24/7 sur FTMO (hors weekend guard) → si aucune barre BTCUSD ou ETHUSD fermée dans les 35 dernières minutes ET on n'est pas en weekend guard (vendredi soir UTC → dimanche soir), c'est que **le feed est mort silencieusement**. Vérifier aussi `journalctl --user -u arabesque-live.service --since "35 minutes ago" | grep -E "(ALREADY_LOGGED_IN|TimeoutError.*PriceFeed|feed.*stale|reconnect.*PriceFeed)"` — si ≥ 1 match → escalader vers trigger `feed_stale` (§3). Incident fondateur : 2026-05-12 23:01 → 2026-05-13 07:54 — cTrader feed FTMO bloqué en boucle `ALREADY_LOGGED_IN` pendant 8h, engine systemd `active`, /suivi disait « tout va bien ». La règle : `is-active=active` + 0 barre crypto ≠ "tout va bien", c'est une alerte critique.
...
| `feed_stale` | **Détection silencieuse — engine actif mais feed mort**. Critère : aucune barre `BTCUSD` ou `ETHUSD` fermée par BarAggregator FTMO dans les 35 dernières minutes (hors fenêtre weekend guard vendredi 22:00 UTC → dimanche 22:00 UTC) **OU** ≥ 1 occurrence `ALREADY_LOGGED_IN` / `TimeoutError.*PriceFeed` / `feed.*stale` dans les logs systemd des 35 dernières minutes. | 🚨 Alerte critique Telegram+ntfy + escalade en **état `🚨 Alerte`** dans l'output (jamais cas A). Ligne `Reco: vérifie l'état du PriceFeed et redémarre l'engine si nécessaire (\`systemctl --user restart arabesque-live.service\`)`. Ne **JAMAIS** auto-restart sans validation user (un restart peut interrompre un trade en cours). |
...
| `selection_coverage` | **Décompose l'écart live↔backtest en COUVERTURE / VARIANCE / BIAIS** (complète `edge_audit_drift` qui dit *que* ça dérive, et `missing_trades_unjustified` qui compte les ratés sans les pondérer par leur R). Lance `python scripts/selection_coverage.py --since 2026-05-16T08:44`. Énumère la population théorique extension+glissade (targets live), calcule `r_theo` (`simulate_pure`), matche pris vs ratés, compare leur R moyen. Verdicts : `selection_bias` (écart raté−pris > 0.30R sur n_pris≥20 → le live rate **systématiquement** les bons signaux : filtre live-only à corriger — ordering cap / spread / slippage / cooldown) ; `mild_tilt` (0.15–0.30R → surveiller) ; `low_coverage_variance` (≈ → sous-échantillonnage/variance, le cap aide, la moyenne de **population** reste la référence d'edge) ; `low_n` (n_pris<15). | Si `selection_bias` → alerte Telegram+ntfy + investiguer le filtre live-only qui jette les bons signaux (ne pas auto-corriger). Sinon inclure **couverture %** + écart dans la notif (informatif : une couverture basse = résultat live dominé par la variance, pas par l'edge). Persiste `logs/selection_coverage_latest.md` + `.jsonl`. **Référence 2026-06-20 (post cap=7)** : `low_coverage_variance`, couverture **29%**, écart **+0.10R** → pas de biais. **RE-MESURE à n_pris≥40** pour confirmer la convergence vers la population post-cap=7 ; si l'écart franchit 0.30R à couverture plus haute, **alors** biais réel. Source : `docs/VALIDATION_CONTRACT.md`. |
