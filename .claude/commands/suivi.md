...
| `edge_decomposition` | **POURQUOI ça dérive** (complète `edge_audit_drift` qui dit *que* ça dérive). Lance **systématiquement** `python scripts/edge_decomposition.py --since J-30` (ou `--since 2026-05-07T23:45` en Phase 4). Catégories quantifiées : `be_missed`, `sl_slipped`, `reconciled`, `mfe_zero_loser`, `wide_spread` + résiduel régime non imputé. Triggers : (a) `be_missed` ≥ 10% de \|ΔExp\| → bug BE non armé, escalader vers `execution_invariants` ; (b) `sl_slipped` ≥ 15% → slippage SL anormal, vérifier guards ; (c) `wide_spread` ≥ 10% → coût exécution, regarder broker ; (d) `reconciled+mfe_zero` borne sup ≥ 30% de \|ΔExp\| → bug tracking probable, prioriser tâche #18/#15 ; (e) résiduel régime ≥ 70% de \|ΔExp\| ET ΔExp < -0.20 → pas un bug, c'est le marché — proposer `/bilan` régime + revue baseline. | Inclure les 2-3 plus grosses catégories dans le résumé Telegram+ntfy (ex: « ΔExp=-0.51R · be_missed=5% · résiduel régime=95% → c'est le marché, pas un bug »). Persiste auto dans `logs/edge_decomposition.jsonl`. **Ne jamais** classer un cost imputable comme régime — chaque catégorie a sa lecture. |
| `replay_drift_live_vs_theory` | **MESURE DIRECTE de l'exécution** (sépare exécution / régime / bias BT). Lance `python scripts/replay_live_vs_theory.py --since J-30` (ou `--since 2026-05-07T23:45` en Phase 4). Pour chaque trade : retrouve la barre signal en parquet, simule trade pur (BE 0.3R / offset 0.20R / TP 2R / SL signal.sl), reporte `Δ_R = R_live − R_theo`. Triggers **par stratégie** (pas global, pour ne pas diluer) : (a) `meanΔR < -0.20R sur n ≥ 20 trades` → exécution mange l'edge, **proposer ultra-rodage ×0.10** sur la stratégie ; (b) `meanΔR < -0.10R sur n ≥ 30 trades` → drift exécution modéré, alerter mais pas auto-action ; (c) `meanΔR ∈ [-0.10R, +0.10R]` → exécution propre, le drift restant est régime ou bias BT (non corrigeable côté live). | Lecture brute par stratégie dans la notif Telegram+ntfy (ex: « Cabriole Δ=+0.02R 31t (propre) · Extension Δ=-0.34R 12t (exécution dévorante → propose ultra-rodage) »). Persiste `logs/replay_live_vs_theory.jsonl` (résumé) + `logs/replay_live_vs_theory_trades.jsonl` (par trade). **Recommandation, pas auto-action** : l'humain change `strategies_ultra` dans `config/settings.yaml` puis restart engine. |
