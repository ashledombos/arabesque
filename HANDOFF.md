...
> 2026-05-27 - **Securite du prochain ordre GFT codee, non chargee en live tant qu'aucun restart n'est decide**. Le prochain runtime integrera : quote REST TradeLocker pre-envoi obligatoire, refus si derive defavorable `>0.25R` ou seuil ATR depasse ; verification SL/TP lies immediatement apres fill, amend unique si necessaire, quarantaine des nouvelles entrees GFT si protection non confirmee ; un fill extreme reste desormais journalise/monitore au lieu d'etre abandonne. Le `protection_check` rejoint le journal ; `broker_guard_rejects.jsonl` est lu par le replay pour ne pas classer un rejet de securite comme panne. Ne pas restart seulement pour charger ce patch pendant les positions XAU ouvertes ; le charger au prochain restart controle ou plus tot uniquement si incident. Doctrine centralisee dans `docs/VALIDATION_CONTRACT.md`; shadow reference complet documente comme chantier requis avant affirmation de rentabilite/hausse de risque.
...
```
🟢 LIVE ACTIF — reprise controlee 2026-05-27 15:41:56 CEST, PID 75323
  Validation : FTMO/GFT plats avant start ; 31/31 souscrit ; moteur pret ; watchdog OK ; health report CAUTION.
  Garde nouvelle : ordre bloque si minimum/step broker impose >1.25x le budget cible.
  Positions live ouvertes depuis reprise : `XAUUSD SHORT` Extension FTMO+GFT, puis `BTCUSD SHORT` Glissade FTMO ; protection broker GFT XAU verifiee.
  Surveillance immediate : sorties/SL/TP des positions, feed/watchdog ; fix pending/reconcile pousse mais a charger uniquement apres retour a plat.
