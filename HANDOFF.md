...
5. **Étages 2/3/4 résilience broker (décision différée après 24-48 h d'observation des étages 0+1)** — Incident DASHUSD 2026-05-20 : `_connected=False` silencieux côté canal trading cTrader pendant ~10 h, amend BE/TSL impossible, position laissée avec SL d'origine pendant tout le retracement de +1.82R → flottant négatif. Étages 0+1 déployés (alerte amend_failures + reconnect-on-demand avant les 4 lignes "Not connected" dans `ctrader.py`) couvrent ~95 % du cas (perte session protobuf récupérable par re-login OAuth déjà en cache). Étages 2/3/4 à décider après observation : (2) healthcheck proactif 5 min côté canal trading (heartbeat ping protobuf pour détecter la perte avant un ordre) ; (3) auto-restart systemd via 2e watchdog timer + `tmp/broker_disconnect_since.txt` si le broker reste injoignable > 10 min ; (4) anti-boucle guard (> 3 restarts/h → stop auto-restart + alerte critique). Critère de décision : si les étages 0+1 résolvent tous les incidents sur 24-48 h → différer 2/3/4 indéfiniment ; si un incident persiste malgré reconnect-on-demand → activer 2 ; si le canal reste mort > 10 min sans qu'un ordre essaie de passer → activer 3+4. Référence incident : log `mai 21 01:42:18: [Monitor] ⚠️ SL amend ABANDONED after 3 attempts: DASHUSD 53110148 target_sl=49.94 last_error=[Not connected]`.
