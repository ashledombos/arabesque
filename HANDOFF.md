...
>
> Derniere mise a jour : 2026-05-29 (soir) - **Patch BE observabilite + equity cTrader cross-currency pret a charger**. Suite a la semaine perdante, l'audit a separe regime defavorable et execution : `replay_live_vs_theory --since 2026-05-26` donne `meanDelta=-0.108R`, avec XRPUSD FTMO comme cas BE theorique non arme live. Correctif local : le polling BE emet maintenant `be_polling_decision` par position evaluee (`armed`, `not_eligible`, `eligible_not_armed` + raison), sans changer les seuils BE/trailing. Autre bug confirme : l'equity cTrader calculait les cross en devise de cotation comme si c'etait de l'USD ; AUDJPY FTMO a donc cree un faux `DANGER` (JPY lu comme USD). Correctif local : equity via `pip_size`/`pip_value_per_lot` calibres de `instruments.yaml`, meme convention que sizing/post-fill. Tests : `317 passed, 1 warning`. A committer/pusher puis restart controle si les positions sont protegees/connues.
