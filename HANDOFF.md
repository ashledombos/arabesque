...
- [ ] **🔬 P2 — Audit tail-risk crypto corrélé (Sonnet, prérequis cap=7)** — pas de dépendance data, faisable de suite. Mesurer expo simultanée LONG/SHORT crypto H4, pire open risk crypto agrégé, scénarios stress (gap corrélé, spread widening) à 5/7/10, comparer sous stress. **À FAIRE AVANT 2026-06-20** (ne pas stagner la décision cap). Gate de cap 5→7. Critères/rollback : `docs/DECISIONS.md` (Décision 2026-06-13).
- [ ] **Décision cap `max_open_positions`** — séquencée : tail-risk OK (P2) → proposer 5→**7** (pas 10), flat + aucun incident + commit config séparé + restart contrôlé → observer 2 sem OU 30 trades propres, rollback si DD>-8% / worst-day<-10R / open_risk>1.5% / invariants alert / perte corrélée crypto (≥3 SL crypto même jour) → 10 seulement après validation cap=7. Détail : `docs/DECISIONS.md` (Décision 2026-06-13).
