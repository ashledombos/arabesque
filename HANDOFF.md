...
>
> Dernière mise à jour : 2026-05-15 — **Watchdog feed v1 actif** (détection panne silencieuse, pas d'auto-restart). Incident PriceFeed 02:03→07:42 UTC + **09:47→17:23 UTC** (fenêtre élargie révélée par diagnostic Glissade XAUUSD 2026-05-15 — 7h36 de boucle Feed stale ETHUSD au lieu des 30min documentés initialement). Réconcilié 2 exits broker (ETHUSD cabriole, XAUUSD glissade) au restart. **Trade Glissade XAUUSD 09:01 UTC = -1R imputable 100% à la panne** (BE jamais armé côté broker car position_monitor sans ticks pendant 7h30). Sans cet outlier, mean ΔR Phase 4 serait positif. 2 signaux Fouetté M1 le 13-05 non tirés (à investiguer). **Audit pipeline validation 2026-05-15 appliqué** : timeframe unifié dans scripts d'audit (#1), 9 aliases CCXT_MAP (#2), strict-data mode `--allow-yahoo` sur replay_signals_vs_live + replay_live_vs_theory (#3), tests cohérence config↔validation (#7, 7 tests), dryrun.py marqué legacy (#5a), pytest config (20 tests OK). meanΔR Phase 4 brut : -0.178R → -0.007R post-fix.
...
- [ ] **Bug `_reconcile_missed_exits` invente exit quand broker indisponible (2026-05-14)** — Au 1er restart 07:39 UTC pendant la panne, FTMO inaccessible (5/5 `ALREADY_LOGGED_IN`+`CANT_ROUTE`). Pourtant `live._reconcile_missed_exits` a logué un faux `exit reconciled_stop_loss` sur ETHUSD SHORT cabriole avec `src=estimated_fallback bars=880` et `mfe_r=0.0` `be_set=False`. La position était toujours ouverte côté broker (confirmée au 2e restart 07:42 UTC). Corrigé manuellement dans `logs/trade_journal.jsonl` : event renommé en `exit_invalidated_by_bug` + ligne `correction_note` appended. **Cause racine probable** : quand `get_closed_position_detail()` renvoie None/timeout (broker indispo), le fallback ne distingue pas "position fermée mais détails indisponibles" de "broker injoignable" → l'estimated_fallback rentre alors qu'il faudrait simplement ne RIEN reconcile. Zone Opus (`arabesque/execution/live.py:_reconcile_missed_exits` + `_reconstruct_exit_from_history`). À patcher : si broker inaccessible (auth error / TCP timeout / connection refused) → différer reconcile au prochain cycle, ne pas inventer.
...
- [ ] **Suspect : XAUUSD glissade reconciled -1R MFE=0.91R BE armé (2026-05-14 19:24 UTC)** — réconcilié au restart, `MFE=0.91R BE=✓ src=broker_detail`. Si BE était armé (à 0.3R = entry+0.20R), un retour à entry aurait dû fermer à +0.20R, pas -1R. Sauf si broker_detail rapporte le close réel au SL signal avant le BE arm tick-level. Vérifier : `arabesque/execution/live._reconstruct_exit_from_history()` — quand broker_detail dit `exit_price=SL signal`, est-ce qu'on garde l'exit=-1R ou est-ce qu'on inférerait BE depuis le MFE ? Si le 2nd, le bug est dans la classification. Si le 1er, comportement ok (broker a vraiment SL, BE pas armé en pratique au moment du tick). Cf. l'incident fondateur 2026-05-07 : 17 mfe_zero_loser → 5 auraient été BE — c'est exactement le pattern qu'on cherche à exclure.
...
| `arabesque-feed-watchdog.timer` | timer toutes les 5min (Persistent) | Détection panne silencieuse PriceFeed (engine `active` + 0 barre fermée > 15min) → alerte Telegram+ntfy. Source `scripts/feed_watchdog.py`, units dans `~/.config/systemd/user/`. Externe au process Python (échappe aux bugs du feed lui-même). Pas d'auto-restart. |
