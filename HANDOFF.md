...
>
> Derniere mise a jour : 2026-05-26 (soir) - **Incident GFT position-state corrige + relecture guards live**. `AUDJPY` est restee ouverte hors monitor apres un HTTP 429 traite comme liste vide puis un faux fill de sortie pris sur l'ordre d'entree ; cloture operateur @ 114.167 et exit reel reconcilie `-0.201R`. `XAUUSD` pending rempli etait invisible car TradeLocker utilise `order_id != position_id`, puis auto-ferme orphelin ; trade restaure `+1.178R` / `MFE=1.79R`. Patch position-state : erreurs `get_positions` deviennent unknown, validation stricte du close fill, resolution pending order->position, restauration SL/TP via ordres lies au restart, correction lifecycle health loop. Relecture : `health_report` melangeait niveau global/equities multi-broker ; correction codee. `cabriole` desactivee conservait 8 pertes consecutives et maintenait a tort `DANGER` ; guard rescope a Extension/Glissade/Fouette (etat attendu apres chargement : `CAUTION`, car Glissade reste a 5 pertes). `OrderDispatcher` evaluait les ordres GFT sur l'etat/config FTMO et n'activait pas `worst_case_budget` en live : gate par broker fail-closed ajoute, avec risk_cash par compte. Relecture complementaire : les pending STOP/LIMIT sont desormais reserves dans l'exposition/daily slots ; toute lecture broker/journal inconnue ou pending broker non trace invalide le gate ; expiration 24h exige confirmation broker ; les messages startup ne peuvent plus annoncer `0 position` sur lecture echouee. **Moteur volontairement arrete : redemarrage a arbitrer explicitement car charger la correction de scope Cabriole ferait passer le guard attendu de `DANGER x0.25` a `CAUTION x0.50`.** Dossier : `docs/INCIDENT_GFT_POSITION_STATE_2026-05-26.md`, decisions du 26 mai.
> 2026-05-27 10:08 CEST - **Reboot pendant maintenance**. Apres coupure reseau/reboot, `arabesque-live.service` encore enabled est parti seul a 10:04 CEST. Il n'a jamais atteint `Moteur pret` : DNS temporairement absent puis boucles cTrader `ALREADY_LOGGED_IN`/timeouts. Watchdog a notifie `no_bar_data_in_window` a 10:06. Intervention : stop a 10:07:09, puis `systemctl --user disable arabesque-live.service`. Verification brokers : FTMO `0 position / 0 pending`, GFT `0 position / 0 pending`. Diagnostic : le timeout cTrader etait nettoye minimalement puis retente apres 5s, ce qui autorise une authentification tardive a creer `ALREADY_LOGGED_IN`. Correction livree : cleanup complet sur timeout + grace startup cTrader >=60s (`38b7277`, `278 passed`). Audit watchdog consolide : l'auto-restart sur `feed_stale` est une decision Hot Path du 23/05 (restaurer le monitoring d'une position), pas un reliquat accidentel ; moteur inactif = aucun start par watchdog. Relecture guard : les pertes miroir FTMO/GFT alimentaient une serie globale malgre la protection par broker ; correction dediee testee (`281 passed`). Recalcul : FTMO `NORMAL`, GFT `CAUTION` via Glissade streak=5 ; la politique pire broker conserve donc le sizing effectif `CAUTION x0.50`. Lecture sizing : 6/7 entries historiques Phase 4 bis sont sous 5% du nominal, mais elles precedent le fix sizing per-broker `ab5b81a`. Projection post-fix en `CAUTION` : Extension H1 ~`31$` FTMO / `73$` GFT ; Glissade rodee ~`7.8$` / `18.2$`. Un faible montant cash ne disqualifie pas l'edge en R sans preuve de distorsion min-volume/spread/commission. **Ne pas re-enable/start** avant audit d'execution sizing et regle de reprise explicite.
...
```
🛑 LIVE SUSPENDU — engine arrêté et service disabled depuis 2026-05-27 10:07 CEST
  Motif courant : maintenance sécurité post-incident GFT + auto-start imprévu au reboot.
  Dernière vérification broker (2026-05-27 10:07 CEST) : FTMO 0 position/0 pending, GFT 0 position/0 pending.
  Blocages avant reprise : audit d'execution sizing (min-volume, risque reel/cible, friction en R) et règle de reprise avant re-enable.
...
- [ ] **PHASE 4 bis (reprise suspendue)** — Scope de verdict Extension+Glissade depuis 2026-05-16. Fix startup cTrader livre (`38b7277`) et correction streak par broker testee (`281 passed`) : GFT Glissade justifie reellement `CAUTION`, donc pas de hausse du risque effectif. Ne pas redémarrer avant audit sizing : les entries historiques tres petites precedent le fix per-broker `ab5b81a`; le point a verifier est la distorsion executee (minimum/step volume, risque reel/cible, friction en R), pas le cash faible en lui-meme. La politique watchdog `feed_stale` est consolidee par Hot Path Mode du 23/05. Critère go ultérieur : `python scripts/check_execution_invariants.py --since 2026-05-16T08:44 --per-broker` → verdict OK sur les 2 brokers.
