...
- [ ] **PHASE 4 bis (reprise suspendue)** — Scope de verdict Extension+Glissade depuis 2026-05-16. Fix startup cTrader livre (`38b7277`) et correction streak par broker livree (`8d3819f`) : GFT Glissade justifie reellement `CAUTION`, donc pas de hausse du risque effectif. Audit sizing fait : 1/7 sur-risque materiel, GFT Glissade XAUUSD au minimum (`9.54x` cible) ; barriere pre-envoi `max_executed_risk_ratio=1.25` codee et a pousser. Apres livraison et verification comptes plats, reprise controlee en `CAUTION` permise avec surveillance des rejets overshoot. La politique watchdog `feed_stale` est consolidee par Hot Path Mode du 23/05. Critère go ultérieur : `python scripts/check_execution_invariants.py --since 2026-05-16T08:44 --per-broker` → verdict OK sur les 2 brokers.
