...
- [ ] **Décision cap `max_open_positions` 5→7** — ✅ **P2 tail-risk levé (cap=7 acceptable)**. Reste à APPLIQUER quand : compte flat + aucun incident actif + commit config séparé + restart contrôlé (validation opérateur). Puis observer 2 sem OU 30 trades propres ; rollback à 5 si DD>-8% / worst-day<-10R / open_risk>1.5% / invariants alert / perte corrélée crypto (≥3 SL crypto même jour). **cap=10 DIFFÉRÉ** jusqu'à reprise DD FTMO (~-5%) ET validation cap=7. Détail : `docs/DECISIONS.md` (Décision 2026-06-13) + `docs/audit/tailrisk_crypto_2026-06-13.md`.
